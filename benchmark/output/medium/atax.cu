...
__device__ float A[392][412];
__device__ float tmp[392];
__device__ float x[412];
__device__ float y[392];
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (392)* (412));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (392)* (412));
	float* h_tmp = (float*) malloc(sizeof(float)* (392));
	cudaMemcpyFromSymbol(h_tmp,tmp,sizeof(float)* (392));
	float* h_x = (float*) malloc(sizeof(float)* (412));
	cudaMemcpyFromSymbol(h_x,x,sizeof(float)* (412));
	float* h_y = (float*) malloc(sizeof(float)* (392));
	cudaMemcpyFromSymbol(h_y,y,sizeof(float)* (392));
	gettimeofday(&t2, 0);
