...
__device__ float A[52][42][62];
__device__ float C4[62][62];
__device__ float sum_[62];
...
__global__ void kernel_5(){
	...
	int thread_count_4 = (40-1)-0+1;
	kernel_4<<<ceil( 1.0 * thread_count_4/1024),1024>>>(i);
	cudaDeviceSynchronize();
}

__global__ void main_kernel(){
	int thread_count_5 = (50-1)-0+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (52)* (42)* (62));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (52)* (42)* (62));
	float* h_C4 = (float*) malloc(sizeof(float)* (62)* (62));
	cudaMemcpyFromSymbol(h_C4,C4,sizeof(float)* (62)* (62));
	float* h_sum_ = (float*) malloc(sizeof(float)* (62));
	cudaMemcpyFromSymbol(h_sum_,sum_,sizeof(float)* (62));
	gettimeofday(&t2, 0);
