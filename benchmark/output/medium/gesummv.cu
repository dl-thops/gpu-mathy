...
__device__ float A[252][252];
__device__ float B[252][252];
__device__ float tmp[252];
__device__ float x[252];
__device__ float y[252];
...
__global__ void kernel_2(float alpha,float beta){
	...
	if( !( 0<=i ) || !( i<=(250-1) ) )return;
	int thread_count_1 = (250-1)-0+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (252)* (252));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (252)* (252));
	float* h_B = (float*) malloc(sizeof(float)* (252)* (252));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (252)* (252));
	float* h_tmp = (float*) malloc(sizeof(float)* (252));
	cudaMemcpyFromSymbol(h_tmp,tmp,sizeof(float)* (252));
	float* h_x = (float*) malloc(sizeof(float)* (252));
	cudaMemcpyFromSymbol(h_x,x,sizeof(float)* (252));
	float* h_y = (float*) malloc(sizeof(float)* (252));
	cudaMemcpyFromSymbol(h_y,y,sizeof(float)* (252));
	gettimeofday(&t2, 0);
