...
__device__ float A[43][43][43];
__device__ float B[43][43][43];
...
__global__ void kernel_2(int i){
	...
	if( !( 1<=j ) || !( j<=(40-1) ) )return;
	int thread_count_1 = (40-1)-1+1;
...
__global__ void kernel_3(){
	...
	if( !( 1<=i ) || !( i<=(40-1) ) )return;
	int thread_count_2 = (40-1)-1+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (43)* (43)* (43));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (43)* (43)* (43));
	float* h_B = (float*) malloc(sizeof(float)* (43)* (43)* (43));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (43)* (43)* (43));
	gettimeofday(&t2, 0);
