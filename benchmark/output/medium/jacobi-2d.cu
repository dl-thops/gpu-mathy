...
__device__ float A[253][253];
__device__ float B[253][253];
...
__global__ void kernel_2(){
	...
	if( !( 1<=i ) || !( i<=(250-1) ) )return;
	int thread_count_1 = (250-1)-1+1;
...
__global__ void kernel_4(){
	...
	if( !( 1<=p ) || !( p<=(250-1) ) )return;
	int thread_count_3 = (250-1)-1+1;
...
__global__ void kernel_5(){
	...
	if( !( 0<=t ) || !( t<=(100-1) ) )return;
	int thread_count_2 = (250-1)-1+1;
	kernel_2<<<ceil( 1.0 * thread_count_2/1024),1024>>>();
	cudaDeviceSynchronize();
	int thread_count_4 = (250-1)-1+1;
	kernel_4<<<ceil( 1.0 * thread_count_4/1024),1024>>>();
	cudaDeviceSynchronize();
}

__global__ void main_kernel(){
	int thread_count_5 = (100-1)-0+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (253)* (253));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (253)* (253));
	float* h_B = (float*) malloc(sizeof(float)* (253)* (253));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (253)* (253));
	gettimeofday(&t2, 0);
