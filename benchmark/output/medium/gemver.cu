...
__device__ float u1[402];
__device__ float u2[402];
__device__ float v1[402];
__device__ float v2[402];
__device__ float w[402][402];
__device__ float x[402];
__device__ float y[402];
__device__ float z[402];
...
__global__ void kernel_7(float alpha){
	int i = 0 + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 0<=i ) || !( i<=(400-1) ) )return;
	int thread_count_6 = (400-1)-0+1;
	float* temp_3 = (float*)malloc(sizeof(float)*((400-1)-0+1));
...
__global__ void main_kernel(){
	...
	cudaDeviceSynchronize();
	int thread_count_5 = (400-1)-0+1;
	kernel_5<<<ceil( 1.0 * thread_count_5/1024),1024>>>();
	cudaDeviceSynchronize();
	int thread_count_7 = (400-1)-0+1;
...
int main(){
	...
	float* h_u1 = (float*) malloc(sizeof(float)* (402));
	cudaMemcpyFromSymbol(h_u1,u1,sizeof(float)* (402));
	float* h_u2 = (float*) malloc(sizeof(float)* (402));
	cudaMemcpyFromSymbol(h_u2,u2,sizeof(float)* (402));
	float* h_v1 = (float*) malloc(sizeof(float)* (402));
	cudaMemcpyFromSymbol(h_v1,v1,sizeof(float)* (402));
	float* h_v2 = (float*) malloc(sizeof(float)* (402));
	cudaMemcpyFromSymbol(h_v2,v2,sizeof(float)* (402));
	float* h_w = (float*) malloc(sizeof(float)* (402)* (402));
	cudaMemcpyFromSymbol(h_w,w,sizeof(float)* (402)* (402));
	float* h_x = (float*) malloc(sizeof(float)* (402));
	cudaMemcpyFromSymbol(h_x,x,sizeof(float)* (402));
	float* h_y = (float*) malloc(sizeof(float)* (402));
	cudaMemcpyFromSymbol(h_y,y,sizeof(float)* (402));
	float* h_z = (float*) malloc(sizeof(float)* (402));
	cudaMemcpyFromSymbol(h_z,z,sizeof(float)* (402));
	gettimeofday(&t2, 0);
