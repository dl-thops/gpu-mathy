...
__device__ float a[182][202];
__device__ float ab[182][192];
__device__ float abcd[182][212];
__device__ float b[202][192];
__device__ float c[192][222];
__device__ float cd[192][212];
__device__ float d[222][212];
...
__global__ void kernel_1(int i,int j,float* temp_1){
	...
	if( !( 0<=k ) || !( k<=(200-1) ) )return;
	temp_1[k-0] = a[i][k] * b[k][j];
}

__global__ void kernel_2(int i){
	int j = 0 + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 0<=j ) || !( j<=(190-1) ) )return;
	int thread_count_1 = (200-1)-0+1;
	float* temp_1 = (float*)malloc(sizeof(float)*((200-1)-0+1));
...
__global__ void kernel_3(){
	...
	if( !( 0<=i ) || !( i<=(180-1) ) )return;
	int thread_count_2 = (190-1)-0+1;
...
__global__ void kernel_4(int i,int j,float* temp_2){
	int k = 0 + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 0<=k ) || !( k<=(220-1) ) )return;
	temp_2[k-0] = c[i][k] * d[k][j];
}

__global__ void kernel_5(int i){
	int j = 0 + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 0<=j ) || !( j<=(210-1) ) )return;
	int thread_count_4 = (220-1)-0+1;
	float* temp_2 = (float*)malloc(sizeof(float)*((220-1)-0+1));
...
__global__ void kernel_6(){
	int i = 0 + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 0<=i ) || !( i<=(190-1) ) )return;
	int thread_count_5 = (210-1)-0+1;
...
__global__ void kernel_8(int i){
	int j = 0 + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 0<=j ) || !( j<=(210-1) ) )return;
	int thread_count_7 = (190-1)-0+1;
	float* temp_3 = (float*)malloc(sizeof(float)*((190-1)-0+1));
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_a = (float*) malloc(sizeof(float)* (182)* (202));
	cudaMemcpyFromSymbol(h_a,a,sizeof(float)* (182)* (202));
	float* h_ab = (float*) malloc(sizeof(float)* (182)* (192));
	cudaMemcpyFromSymbol(h_ab,ab,sizeof(float)* (182)* (192));
	float* h_abcd = (float*) malloc(sizeof(float)* (182)* (212));
	cudaMemcpyFromSymbol(h_abcd,abcd,sizeof(float)* (182)* (212));
	float* h_b = (float*) malloc(sizeof(float)* (202)* (192));
	cudaMemcpyFromSymbol(h_b,b,sizeof(float)* (202)* (192));
	float* h_c = (float*) malloc(sizeof(float)* (192)* (222));
	cudaMemcpyFromSymbol(h_c,c,sizeof(float)* (192)* (222));
	float* h_cd = (float*) malloc(sizeof(float)* (192)* (212));
	cudaMemcpyFromSymbol(h_cd,cd,sizeof(float)* (192)* (212));
	float* h_d = (float*) malloc(sizeof(float)* (222)* (212));
	cudaMemcpyFromSymbol(h_d,d,sizeof(float)* (222)* (212));
	gettimeofday(&t2, 0);
