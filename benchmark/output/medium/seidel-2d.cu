...
__device__ float A[403][403];
__device__ float A_t[402][402];
...
__global__ void kernel_2(){
	...
	if( !( 1<=i ) || !( i<=(400 - 2-1) ) )return;
	int thread_count_1 = (400 - 2-1)-1+1;
...
__global__ void kernel_4(){
	int i = 1 + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 1<=i ) || !( i<=(400 - 2-1) ) )return;
	int thread_count_3 = (400 - 2-1)-1+1;
...
__global__ void kernel_5(){
	...
	int thread_count_2 = (400 - 2-1)-1+1;
	kernel_2<<<ceil( 1.0 * thread_count_2/1024),1024>>>();
	cudaDeviceSynchronize();
	int thread_count_4 = (400 - 2-1)-1+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (403)* (403));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (403)* (403));
	float* h_A_t = (float*) malloc(sizeof(float)* (402)* (402));
	cudaMemcpyFromSymbol(h_A_t,A_t,sizeof(float)* (402)* (402));
	gettimeofday(&t2, 0);
