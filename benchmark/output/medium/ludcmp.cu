...
__device__ float A[4001][402];
__device__ float b[402];
...
__global__ void kernel_8(int i,float w){
	int j = 400 - i + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 400 - i<=j ) || !( j<=(400-1) ) )return;
	w = w - A[3999-i][j] * x[j];
}
...
__global__ void kernel_9(float w){
	...
	if( !( 0<=i ) || !( i<=(400-1) ) )return;
	w = y[400-1-i];
	int thread_count_8 = (400-1)-400 - i+1;
	kernel_8<<<ceil( 1.0 * thread_count_8/1024),1024>>>(i,w);
	cudaDeviceSynchronize();
	x[400-1-i] = w / A[400-1-i][400-1-i];
}
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (4001)* (402));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (4001)* (402));
	float* h_b = (float*) malloc(sizeof(float)* (402));
	cudaMemcpyFromSymbol(h_b,b,sizeof(float)* (402));
