...
__device__ float A[202][242];
__device__ float Q[202][242];
__device__ float R[242][242];
...
__global__ void kernel_6(float norm_){
	...
	norm_ = temp_1[0];
	R[k][k] = sqrt(norm_);
	int thread_count_2 = (200-1)-0+1;
	kernel_2<<<ceil( 1.0 * thread_count_2/1024),1024>>>(k);
	cudaDeviceSynchronize();
	int thread_count_5 = (240-1)-k + 1+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (202)* (242));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (202)* (242));
	float* h_Q = (float*) malloc(sizeof(float)* (202)* (242));
	cudaMemcpyFromSymbol(h_Q,Q,sizeof(float)* (202)* (242));
	float* h_R = (float*) malloc(sizeof(float)* (242)* (242));
	cudaMemcpyFromSymbol(h_R,R,sizeof(float)* (242)* (242));
	gettimeofday(&t2, 0);
