...
__device__ float A[412][392];
__device__ float p[392];
__device__ float q[412];
__device__ float r[412];
__device__ float s[412];
...
__global__ void kernel_3(){
	...
	if( !( 0<=i ) || !( i<=(410-1) ) )return;
	int thread_count_1 = (390-1)-0+1;
	float* temp_1 = (float*)malloc(sizeof(float)*((390-1)-0+1));
	kernel_1<<<ceil( 1.0 * thread_count_1/1024),1024>>>(i,temp_1);
	cudaDeviceSynchronize();
	sumArray( temp_1,thread_count_1);
	cudaDeviceSynchronize();
	s[i] = temp_1[0];
	int thread_count_2 = (390-1)-0+1;
	float* temp_2 = (float*)malloc(sizeof(float)*((390-1)-0+1));
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (412)* (392));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (412)* (392));
	float* h_p = (float*) malloc(sizeof(float)* (392));
	cudaMemcpyFromSymbol(h_p,p,sizeof(float)* (392));
	float* h_q = (float*) malloc(sizeof(float)* (412));
	cudaMemcpyFromSymbol(h_q,q,sizeof(float)* (412));
	float* h_r = (float*) malloc(sizeof(float)* (412));
	cudaMemcpyFromSymbol(h_r,r,sizeof(float)* (412));
	float* h_s = (float*) malloc(sizeof(float)* (412));
	cudaMemcpyFromSymbol(h_s,s,sizeof(float)* (412));
	gettimeofday(&t2, 0);
