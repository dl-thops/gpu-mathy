...
__device__ float B[82][72];
__device__ float C[62][82];
...
__global__ void kernel_3(float alpha,int i){
	...
	int thread_count_2 = (70-1)-0+1;
	float* temp_1 = (float*)malloc(sizeof(float)*((70-1)-0+1));
...
__global__ void kernel_4(float alpha,float beta){
	...
	int thread_count_1 = (70-1)-0+1;
	kernel_1<<<ceil( 1.0 * thread_count_1/1024),1024>>>(beta,i);
	cudaDeviceSynchronize();
	int thread_count_3 = (80-1)-0+1;
...
int main(){
	...
	float* h_B = (float*) malloc(sizeof(float)* (82)* (72));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (82)* (72));
	float* h_C = (float*) malloc(sizeof(float)* (62)* (82));
	cudaMemcpyFromSymbol(h_C,C,sizeof(float)* (62)* (82));
	gettimeofday(&t2, 0);
