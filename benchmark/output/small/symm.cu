...
__device__ float A[62][62];
__device__ float B[62][82];
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (62)* (62));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (62)* (62));
	float* h_B = (float*) malloc(sizeof(float)* (62)* (82));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (62)* (82));
