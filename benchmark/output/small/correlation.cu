...
__global__ void kernel_7(){
	...
	if( !( 0<=t ) || !( t<=(80-1) ) )return;
	int thread_count_6 = (100-1)-0+1;
	float* temp_3 = (float*)malloc(sizeof(float)*((100-1)-0+1));
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_corr = (float*) malloc(sizeof(float)* (82)* (82));
	cudaMemcpyFromSymbol(h_corr,corr,sizeof(float)* (82)* (82));
	...
	float* h_std_dev_ = (float*) malloc(sizeof(float)* (82));
	cudaMemcpyFromSymbol(h_std_dev_,std_dev_,sizeof(float)* (82));
	gettimeofday(&t2, 0);
