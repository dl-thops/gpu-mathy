...
__global__ void kernel_8(int i,float w){
	int j = 120 - i + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 120 - i<=j ) || !( j<=(120-1) ) )return;
	w = w - A[119-i][j] * x[j];
}
...
__global__ void kernel_9(float w){
	...
	if( !( 0<=i ) || !( i<=(120-1) ) )return;
	w = y[120-1-i];
	int thread_count_8 = (120-1)-120 - i+1;
	kernel_8<<<ceil( 1.0 * thread_count_8/1024),1024>>>(i,w);
	cudaDeviceSynchronize();
	x[120-1-i] = w / A[120-1-i][120-1-i];
}
...
int main(){
	...
	float* h_b = (float*) malloc(sizeof(float)* (122));
	cudaMemcpyFromSymbol(h_b,b,sizeof(float)* (122));
