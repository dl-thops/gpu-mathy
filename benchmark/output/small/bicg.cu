...
__device__ float A[126][118];
__device__ float p[118];
__device__ float q[126];
__device__ float r[126];
__device__ float s[126];
...
__global__ void kernel_3(){
	...
	if( !( 0<=i ) || !( i<=(124-1) ) )return;
	int thread_count_1 = (116-1)-0+1;
	float* temp_1 = (float*)malloc(sizeof(float)*((116-1)-0+1));
	kernel_1<<<ceil( 1.0 * thread_count_1/1024),1024>>>(i,temp_1);
	cudaDeviceSynchronize();
	sumArray( temp_1,thread_count_1);
	cudaDeviceSynchronize();
	s[i] = temp_1[0];
	int thread_count_2 = (116-1)-0+1;
	float* temp_2 = (float*)malloc(sizeof(float)*((116-1)-0+1));
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (126)* (118));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (126)* (118));
	float* h_p = (float*) malloc(sizeof(float)* (118));
	cudaMemcpyFromSymbol(h_p,p,sizeof(float)* (118));
	float* h_q = (float*) malloc(sizeof(float)* (126));
	cudaMemcpyFromSymbol(h_q,q,sizeof(float)* (126));
	float* h_r = (float*) malloc(sizeof(float)* (126));
	cudaMemcpyFromSymbol(h_r,r,sizeof(float)* (126));
	float* h_s = (float*) malloc(sizeof(float)* (126));
	cudaMemcpyFromSymbol(h_s,s,sizeof(float)* (126));
	gettimeofday(&t2, 0);
