...
__device__ float A[23][23][23];
__device__ float B[23][23][23];
...
__global__ void kernel_2(int i){
	...
	if( !( 1<=j ) || !( j<=(20-1) ) )return;
	int thread_count_1 = (20-1)-1+1;
...
__global__ void kernel_3(){
	...
	if( !( 1<=i ) || !( i<=(20-1) ) )return;
	int thread_count_2 = (20-1)-1+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (23)* (23)* (23));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (23)* (23)* (23));
	float* h_B = (float*) malloc(sizeof(float)* (23)* (23)* (23));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (23)* (23)* (23));
	gettimeofday(&t2, 0);
