...
__device__ float a[42][72];
__device__ float b[72][52];
__device__ float c[52][82];
__device__ float d[42][82];
__device__ float temp[42][52];
...
__global__ void kernel_2(float alpha,int i){
	...
	if( !( 0<=j ) || !( j<=50 ) )return;
	int thread_count_1 = 70-0+1;
	float* temp_1 = (float*)malloc(sizeof(float)*(70-0+1));
...
__global__ void kernel_5(float beta,int i){
	...
	if( !( 0<=j ) || !( j<=80 ) )return;
	d[i][j] = d[i][j] * beta;
	int thread_count_4 = 50-0+1;
	float* temp_2 = (float*)malloc(sizeof(float)*(50-0+1));
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_a = (float*) malloc(sizeof(float)* (42)* (72));
	cudaMemcpyFromSymbol(h_a,a,sizeof(float)* (42)* (72));
	float* h_b = (float*) malloc(sizeof(float)* (72)* (52));
	cudaMemcpyFromSymbol(h_b,b,sizeof(float)* (72)* (52));
	float* h_c = (float*) malloc(sizeof(float)* (52)* (82));
	cudaMemcpyFromSymbol(h_c,c,sizeof(float)* (52)* (82));
	float* h_d = (float*) malloc(sizeof(float)* (42)* (82));
	cudaMemcpyFromSymbol(h_d,d,sizeof(float)* (42)* (82));
	float* h_temp = (float*) malloc(sizeof(float)* (42)* (52));
	cudaMemcpyFromSymbol(h_temp,temp,sizeof(float)* (42)* (52));
	gettimeofday(&t2, 0);
