...
__device__ float A[62][82];
__device__ float Q[62][82];
__device__ float R[82][82];
...
__global__ void kernel_5(int k){
	...
	if( !( k + 1<=j ) || !( j<=(80-1) ) )return;
	int thread_count_3 = (60-1)-0+1;
	float* temp_2 = (float*)malloc(sizeof(float)*((60-1)-0+1));
	kernel_3<<<ceil( 1.0 * thread_count_3/1024),1024>>>(j,k,temp_2);
	cudaDeviceSynchronize();
	sumArray( temp_2,thread_count_3);
	cudaDeviceSynchronize();
	R[k][j] = temp_2[0];
	int thread_count_4 = (60-1)-0+1;
...
__global__ void kernel_6(float norm_){
	...
	norm_ = temp_1[0];
	R[k][k] = sqrt(norm_);
	int thread_count_2 = (60-1)-0+1;
	kernel_2<<<ceil( 1.0 * thread_count_2/1024),1024>>>(k);
	cudaDeviceSynchronize();
	int thread_count_5 = (80-1)-k + 1+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (62)* (82));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (62)* (82));
	float* h_Q = (float*) malloc(sizeof(float)* (62)* (82));
	cudaMemcpyFromSymbol(h_Q,Q,sizeof(float)* (62)* (82));
	float* h_R = (float*) malloc(sizeof(float)* (82)* (82));
	cudaMemcpyFromSymbol(h_R,R,sizeof(float)* (82)* (82));
	gettimeofday(&t2, 0);
