...
__device__ float A[2803][2803];
__device__ float B[2803][2803];
...
__global__ void kernel_2(){
	...
	if( !( 1<=i ) || !( i<=(2800-1) ) )return;
	int thread_count_1 = (2800-1)-1+1;
...
__global__ void kernel_4(){
	...
	if( !( 1<=p ) || !( p<=(2800-1) ) )return;
	int thread_count_3 = (2800-1)-1+1;
...
__global__ void kernel_5(){
	...
	int thread_count_2 = (2800-1)-1+1;
	kernel_2<<<ceil( 1.0 * thread_count_2/1024),1024>>>();
	cudaDeviceSynchronize();
	int thread_count_4 = (2800-1)-1+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (2803)* (2803));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (2803)* (2803));
	float* h_B = (float*) malloc(sizeof(float)* (2803)* (2803));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (2803)* (2803));
	gettimeofday(&t2, 0);
