...
__device__ float A[21002][19002];
__device__ float p[19002];
__device__ float q[21002];
__device__ float r[21002];
__device__ float s[21002];
...
__global__ void kernel_3(){
	...
	if( !( 0<=i ) || !( i<=(21000-1) ) )return;
	int thread_count_1 = (19000-1)-0+1;
	float* temp_1 = (float*)malloc(sizeof(float)*((19000-1)-0+1));
	kernel_1<<<ceil( 1.0 * thread_count_1/1024),1024>>>(i,temp_1);
	cudaDeviceSynchronize();
	sumArray( temp_1,thread_count_1);
	cudaDeviceSynchronize();
	s[i] = temp_1[0];
	int thread_count_2 = (19000-1)-0+1;
	float* temp_2 = (float*)malloc(sizeof(float)*((19000-1)-0+1));
...
__global__ void main_kernel(){
	int thread_count_3 = (21000-1)-0+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (21002)* (19002));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (21002)* (19002));
	float* h_p = (float*) malloc(sizeof(float)* (19002));
	cudaMemcpyFromSymbol(h_p,p,sizeof(float)* (19002));
	float* h_q = (float*) malloc(sizeof(float)* (21002));
	cudaMemcpyFromSymbol(h_q,q,sizeof(float)* (21002));
	float* h_r = (float*) malloc(sizeof(float)* (21002));
	cudaMemcpyFromSymbol(h_r,r,sizeof(float)* (21002));
	float* h_s = (float*) malloc(sizeof(float)* (21002));
	cudaMemcpyFromSymbol(h_s,s,sizeof(float)* (21002));
	gettimeofday(&t2, 0);
