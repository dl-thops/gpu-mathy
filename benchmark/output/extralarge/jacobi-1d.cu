...
__device__ float A[200003];
__device__ float B[200003];
...
__global__ void kernel_3(){
	...
	if( !( 0<=t ) || !( t<=(25000-1) ) )return;
	int thread_count_1 = (200000-1)-1+1;
	kernel_1<<<ceil( 1.0 * thread_count_1/1024),1024>>>();
	cudaDeviceSynchronize();
	int thread_count_2 = (200000-1)-1+1;
	kernel_2<<<ceil( 1.0 * thread_count_2/1024),1024>>>();
	cudaDeviceSynchronize();
}

__global__ void main_kernel(){
	int thread_count_3 = (25000-1)-0+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (200003));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (200003));
	float* h_B = (float*) malloc(sizeof(float)* (200003));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (200003));
	gettimeofday(&t2, 0);
