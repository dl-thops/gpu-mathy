...
__device__ float A[4003];
__device__ float B[4003];
...
__global__ void kernel_3(){
	...
	int thread_count_1 = (4000-1)-1+1;
	kernel_1<<<ceil( 1.0 * thread_count_1/1024),1024>>>();
	cudaDeviceSynchronize();
	int thread_count_2 = (4000-1)-1+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (4003));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (4003));
	float* h_B = (float*) malloc(sizeof(float)* (4003));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (4003));
	gettimeofday(&t2, 0);
