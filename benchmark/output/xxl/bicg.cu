...
__device__ float A[9002][8002];
__device__ float p[8002];
__device__ float q[9002];
__device__ float r[9002];
__device__ float s[9002];
...
__global__ void kernel_3(){
	...
	cudaDeviceSynchronize();
	s[i] = temp_1[0];
	int thread_count_2 = (8000-1)-0+1;
	float* temp_2 = (float*)malloc(sizeof(float)*((8000-1)-0+1));
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (9002)* (8002));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (9002)* (8002));
	float* h_p = (float*) malloc(sizeof(float)* (8002));
	cudaMemcpyFromSymbol(h_p,p,sizeof(float)* (8002));
	float* h_q = (float*) malloc(sizeof(float)* (9002));
	cudaMemcpyFromSymbol(h_q,q,sizeof(float)* (9002));
	float* h_r = (float*) malloc(sizeof(float)* (9002));
	cudaMemcpyFromSymbol(h_r,r,sizeof(float)* (9002));
	float* h_s = (float*) malloc(sizeof(float)* (9002));
	cudaMemcpyFromSymbol(h_s,s,sizeof(float)* (9002));
	gettimeofday(&t2, 0);
