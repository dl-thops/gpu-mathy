...
__device__ float u1[12002];
__device__ float u2[12002];
__device__ float v1[12002];
__device__ float v2[12002];
__device__ float w[12002][12002];
__device__ float x[12002];
__device__ float y[12002];
__device__ float z[12002];
...
__global__ void kernel_2(){
	...
	int thread_count_1 = (12000-1)-0+1;
	float* temp_1 = (float*)malloc(sizeof(float)*((12000-1)-0+1));
...
__global__ void kernel_4(float beta){
	int i = 0 + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 0<=i ) || !( i<=(12000-1) ) )return;
	int thread_count_3 = (12000-1)-0+1;
	float* temp_2 = (float*)malloc(sizeof(float)*((12000-1)-0+1));
...
__global__ void kernel_7(float alpha){
	int i = 0 + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 0<=i ) || !( i<=(12000-1) ) )return;
	int thread_count_6 = (12000-1)-0+1;
	float* temp_3 = (float*)malloc(sizeof(float)*((12000-1)-0+1));
...
__global__ void main_kernel(){
	...
	alpha = 1.5;
	beta = 1.2;
	int thread_count_2 = (12000-1)-0+1;
	kernel_2<<<ceil( 1.0 * thread_count_2/1024),1024>>>();
	cudaDeviceSynchronize();
	int thread_count_4 = (12000-1)-0+1;
...
int main(){
	...
	float* h_u1 = (float*) malloc(sizeof(float)* (12002));
	cudaMemcpyFromSymbol(h_u1,u1,sizeof(float)* (12002));
	float* h_u2 = (float*) malloc(sizeof(float)* (12002));
	cudaMemcpyFromSymbol(h_u2,u2,sizeof(float)* (12002));
	float* h_v1 = (float*) malloc(sizeof(float)* (12002));
	cudaMemcpyFromSymbol(h_v1,v1,sizeof(float)* (12002));
	float* h_v2 = (float*) malloc(sizeof(float)* (12002));
	cudaMemcpyFromSymbol(h_v2,v2,sizeof(float)* (12002));
	float* h_w = (float*) malloc(sizeof(float)* (12002)* (12002));
	cudaMemcpyFromSymbol(h_w,w,sizeof(float)* (12002)* (12002));
	...
	float* h_z = (float*) malloc(sizeof(float)* (12002));
	cudaMemcpyFromSymbol(h_z,z,sizeof(float)* (12002));
	gettimeofday(&t2, 0);
