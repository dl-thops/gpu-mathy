...
__device__ float A[7002][9002];
__device__ float B[9002][8002];
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (7002)* (9002));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (7002)* (9002));
	float* h_B = (float*) malloc(sizeof(float)* (9002)* (8002));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (9002)* (8002));
