...
__device__ float Q[6002][8002];
__device__ float R[8002][8002];
...
__global__ void kernel_6(float norm_){
	...
	int thread_count_1 = (6000-1)-0+1;
	float* temp_1 = (float*)malloc(sizeof(float)*((6000-1)-0+1));
	...
	norm_ = temp_1[0];
	R[k][k] = sqrt(norm_);
	int thread_count_2 = (6000-1)-0+1;
	kernel_2<<<ceil( 1.0 * thread_count_2/1024),1024>>>(k);
	cudaDeviceSynchronize();
	int thread_count_5 = (8000-1)-k + 1+1;
...
int main(){
	...
	float* h_Q = (float*) malloc(sizeof(float)* (6002)* (8002));
	cudaMemcpyFromSymbol(h_Q,Q,sizeof(float)* (6002)* (8002));
	float* h_R = (float*) malloc(sizeof(float)* (8002)* (8002));
	cudaMemcpyFromSymbol(h_R,R,sizeof(float)* (8002)* (8002));
	gettimeofday(&t2, 0);
