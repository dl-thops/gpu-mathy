...
__device__ float A[10003][10003];
__device__ float B[10003][10003];
...
__global__ void kernel_2(){
	...
	if( !( 1<=i ) || !( i<=(10000-1) ) )return;
	int thread_count_1 = (10000-1)-1+1;
...
__global__ void kernel_4(){
	...
	if( !( 1<=p ) || !( p<=(10000-1) ) )return;
	int thread_count_3 = (10000-1)-1+1;
...
__global__ void kernel_5(){
	...
	int thread_count_2 = (10000-1)-1+1;
	kernel_2<<<ceil( 1.0 * thread_count_2/1024),1024>>>();
	cudaDeviceSynchronize();
	int thread_count_4 = (10000-1)-1+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (10003)* (10003));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (10003)* (10003));
	float* h_B = (float*) malloc(sizeof(float)* (10003)* (10003));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (10003)* (10003));
	gettimeofday(&t2, 0);
