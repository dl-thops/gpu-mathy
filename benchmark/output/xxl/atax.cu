...
__device__ float A[8002][9002];
__device__ float tmp[8002];
__device__ float x[9002];
__device__ float y[8002];
...
__global__ void kernel_2(int i,float* temp_2){
	...
	if( !( 0<=k ) || !( k<=(9000-1) ) )return;
	temp_2[k-0] = A[i][k] * tmp[i];
}

__global__ void kernel_3(){
	int i = 0 + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 0<=i ) || !( i<=(8000-1) ) )return;
	int thread_count_1 = (9000-1)-0+1;
	float* temp_1 = (float*)malloc(sizeof(float)*((9000-1)-0+1));
	...
	float* temp_2 = (float*)malloc(sizeof(float)*((9000-1)-0+1));
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (8002)* (9002));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (8002)* (9002));
	float* h_tmp = (float*) malloc(sizeof(float)* (8002));
	cudaMemcpyFromSymbol(h_tmp,tmp,sizeof(float)* (8002));
	float* h_x = (float*) malloc(sizeof(float)* (9002));
	cudaMemcpyFromSymbol(h_x,x,sizeof(float)* (9002));
	float* h_y = (float*) malloc(sizeof(float)* (8002));
	cudaMemcpyFromSymbol(h_y,y,sizeof(float)* (8002));
	gettimeofday(&t2, 0);
