...
__device__ float A[902][1102];
__device__ float tmp[902];
__device__ float x[1102];
__device__ float y[902];
...
__global__ void kernel_3(){
	...
	float* temp_2 = (float*)malloc(sizeof(float)*((1100-1)-0+1));
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (902)* (1102));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (902)* (1102));
	float* h_tmp = (float*) malloc(sizeof(float)* (902));
	cudaMemcpyFromSymbol(h_tmp,tmp,sizeof(float)* (902));
	float* h_x = (float*) malloc(sizeof(float)* (1102));
	cudaMemcpyFromSymbol(h_x,x,sizeof(float)* (1102));
	float* h_y = (float*) malloc(sizeof(float)* (902));
	cudaMemcpyFromSymbol(h_y,y,sizeof(float)* (902));
	gettimeofday(&t2, 0);
