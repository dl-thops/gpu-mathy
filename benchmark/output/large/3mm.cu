...
__device__ float a[802][1002];
__device__ float ab[802][902];
__device__ float abcd[802][1102];
__device__ float b[1002][902];
__device__ float c[902][1202];
__device__ float cd[902][1102];
__device__ float d[1202][1102];
...
__global__ void kernel_5(int i){
	...
	int thread_count_4 = (1200-1)-0+1;
	float* temp_2 = (float*)malloc(sizeof(float)*((1200-1)-0+1));
...
__global__ void kernel_6(){
	int i = 0 + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 0<=i ) || !( i<=(900-1) ) )return;
	int thread_count_5 = (1100-1)-0+1;
...
__global__ void kernel_8(int i){
	int j = 0 + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 0<=j ) || !( j<=(1100-1) ) )return;
	int thread_count_7 = (900-1)-0+1;
	float* temp_3 = (float*)malloc(sizeof(float)*((900-1)-0+1));
...
__global__ void main_kernel(){
	...
	cudaDeviceSynchronize();
	int thread_count_6 = (900-1)-0+1;
	kernel_6<<<ceil( 1.0 * thread_count_6/1024),1024>>>();
	cudaDeviceSynchronize();
	int thread_count_9 = (800-1)-0+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_a = (float*) malloc(sizeof(float)* (802)* (1002));
	cudaMemcpyFromSymbol(h_a,a,sizeof(float)* (802)* (1002));
	float* h_ab = (float*) malloc(sizeof(float)* (802)* (902));
	cudaMemcpyFromSymbol(h_ab,ab,sizeof(float)* (802)* (902));
	float* h_abcd = (float*) malloc(sizeof(float)* (802)* (1102));
	cudaMemcpyFromSymbol(h_abcd,abcd,sizeof(float)* (802)* (1102));
	float* h_b = (float*) malloc(sizeof(float)* (1002)* (902));
	cudaMemcpyFromSymbol(h_b,b,sizeof(float)* (1002)* (902));
	float* h_c = (float*) malloc(sizeof(float)* (902)* (1202));
	cudaMemcpyFromSymbol(h_c,c,sizeof(float)* (902)* (1202));
	float* h_cd = (float*) malloc(sizeof(float)* (902)* (1102));
	cudaMemcpyFromSymbol(h_cd,cd,sizeof(float)* (902)* (1102));
	float* h_d = (float*) malloc(sizeof(float)* (1202)* (1102));
	cudaMemcpyFromSymbol(h_d,d,sizeof(float)* (1202)* (1102));
	gettimeofday(&t2, 0);
