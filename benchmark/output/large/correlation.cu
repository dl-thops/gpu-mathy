...
__device__ float corr[1202][1202];
__device__ float cov_[1202][1202];
__device__ float data_[2402][1202];
__device__ float mean_[1202];
__device__ float std_dev_[1202];
...
__global__ void kernel_4(int i){
	...
	if( !( 0<=j ) || !( j<=(1200-1) ) )return;
	int thread_count_3 = (2400-1)-0+1;
...
__global__ void kernel_7(){
	...
	if( !( 0<=t ) || !( t<=(1200-1) ) )return;
	int thread_count_6 = (2400-1)-0+1;
	float* temp_3 = (float*)malloc(sizeof(float)*((2400-1)-0+1));
...
__global__ void main_kernel(){
	...
	cudaDeviceSynchronize();
	int thread_count_7 = (1200-1)-0+1;
	kernel_7<<<ceil( 1.0 * thread_count_7/1024),1024>>>();
	cudaDeviceSynchronize();
	int thread_count_8 = (1200-1)-0+1;
	kernel_8<<<ceil( 1.0 * thread_count_8/1024),1024>>>();
	cudaDeviceSynchronize();
	int thread_count_10 = (1200-1)-0+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_corr = (float*) malloc(sizeof(float)* (1202)* (1202));
	cudaMemcpyFromSymbol(h_corr,corr,sizeof(float)* (1202)* (1202));
	float* h_cov_ = (float*) malloc(sizeof(float)* (1202)* (1202));
	cudaMemcpyFromSymbol(h_cov_,cov_,sizeof(float)* (1202)* (1202));
	float* h_data_ = (float*) malloc(sizeof(float)* (2402)* (1202));
	cudaMemcpyFromSymbol(h_data_,data_,sizeof(float)* (2402)* (1202));
	float* h_mean_ = (float*) malloc(sizeof(float)* (1202));
	cudaMemcpyFromSymbol(h_mean_,mean_,sizeof(float)* (1202));
	float* h_std_dev_ = (float*) malloc(sizeof(float)* (1202));
	cudaMemcpyFromSymbol(h_std_dev_,std_dev_,sizeof(float)* (1202));
	gettimeofday(&t2, 0);
