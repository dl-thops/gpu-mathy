...
__device__ float A[123][123][123];
__device__ float B[123][123][123];
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (123)* (123)* (123));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (123)* (123)* (123));
	float* h_B = (float*) malloc(sizeof(float)* (123)* (123)* (123));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (123)* (123)* (123));
	gettimeofday(&t2, 0);
