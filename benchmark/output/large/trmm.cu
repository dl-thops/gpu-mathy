...
__device__ float A[1002][1002];
__device__ float B[1002][1202];
...
__global__ void kernel_2(float alpha,int i){
	...
	int thread_count_1 = (1000-1)-i + 1+1;
	float* temp_1 = (float*)malloc(sizeof(float)*((1000-1)-i + 1+1));
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (1002)* (1002));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (1002)* (1002));
	float* h_B = (float*) malloc(sizeof(float)* (1002)* (1202));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (1002)* (1202));
	gettimeofday(&t2, 0);
