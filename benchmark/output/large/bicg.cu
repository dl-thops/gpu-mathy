...
__device__ float A[1102][902];
__device__ float p[902];
__device__ float q[1102];
__device__ float r[1102];
__device__ float s[1102];
...
__global__ void kernel_3(){
	...
	float* temp_2 = (float*)malloc(sizeof(float)*((900-1)-0+1));
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (1102)* (902));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (1102)* (902));
	float* h_p = (float*) malloc(sizeof(float)* (902));
	cudaMemcpyFromSymbol(h_p,p,sizeof(float)* (902));
	float* h_q = (float*) malloc(sizeof(float)* (1102));
	cudaMemcpyFromSymbol(h_q,q,sizeof(float)* (1102));
	float* h_r = (float*) malloc(sizeof(float)* (1102));
	cudaMemcpyFromSymbol(h_r,r,sizeof(float)* (1102));
	float* h_s = (float*) malloc(sizeof(float)* (1102));
	cudaMemcpyFromSymbol(h_s,s,sizeof(float)* (1102));
	gettimeofday(&t2, 0);
