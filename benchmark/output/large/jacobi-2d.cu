...
__device__ float A[1303][1303];
__device__ float B[1303][1303];
...
__global__ void kernel_2(){
	...
	if( !( 1<=i ) || !( i<=(1300-1) ) )return;
	int thread_count_1 = (1300-1)-1+1;
...
__global__ void kernel_4(){
	...
	if( !( 1<=p ) || !( p<=(1300-1) ) )return;
	int thread_count_3 = (1300-1)-1+1;
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_A = (float*) malloc(sizeof(float)* (1303)* (1303));
	cudaMemcpyFromSymbol(h_A,A,sizeof(float)* (1303)* (1303));
	float* h_B = (float*) malloc(sizeof(float)* (1303)* (1303));
	cudaMemcpyFromSymbol(h_B,B,sizeof(float)* (1303)* (1303));
	gettimeofday(&t2, 0);
