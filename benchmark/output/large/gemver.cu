...
__device__ float u1[2002];
__device__ float u2[2002];
__device__ float v1[2002];
__device__ float v2[2002];
__device__ float w[2002][2002];
__device__ float x[2002];
__device__ float y[2002];
__device__ float z[2002];
...
__global__ void kernel_4(float beta){
	...
	float* temp_2 = (float*)malloc(sizeof(float)*((2000-1)-0+1));
...
__global__ void kernel_7(float alpha){
	int i = 0 + blockDim.x * blockIdx.x + threadIdx.x;
	if( !( 0<=i ) || !( i<=(2000-1) ) )return;
	int thread_count_6 = (2000-1)-0+1;
	float* temp_3 = (float*)malloc(sizeof(float)*((2000-1)-0+1));
...
int main(){
	...
	float* h_u1 = (float*) malloc(sizeof(float)* (2002));
	cudaMemcpyFromSymbol(h_u1,u1,sizeof(float)* (2002));
	float* h_u2 = (float*) malloc(sizeof(float)* (2002));
	cudaMemcpyFromSymbol(h_u2,u2,sizeof(float)* (2002));
	float* h_v1 = (float*) malloc(sizeof(float)* (2002));
	cudaMemcpyFromSymbol(h_v1,v1,sizeof(float)* (2002));
	float* h_v2 = (float*) malloc(sizeof(float)* (2002));
	cudaMemcpyFromSymbol(h_v2,v2,sizeof(float)* (2002));
	float* h_w = (float*) malloc(sizeof(float)* (2002)* (2002));
	cudaMemcpyFromSymbol(h_w,w,sizeof(float)* (2002)* (2002));
	...
	float* h_z = (float*) malloc(sizeof(float)* (2002));
	cudaMemcpyFromSymbol(h_z,z,sizeof(float)* (2002));
	gettimeofday(&t2, 0);
