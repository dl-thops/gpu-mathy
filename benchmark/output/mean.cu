...
__global__ void kernel_1(float* temp_1){
...
__global__ void main_kernel(){
	...
	kernel_1<<<ceil( 1.0 * thread_count_1/1024),1024>>>(temp_1);
	cudaDeviceSynchronize();
...
int main(){
	...
	main_kernel<<<1,1>>>();
	cudaDeviceSynchronize();
	float* h_a = (float*) malloc(sizeof(float)* (102));
	cudaMemcpyFromSymbol(h_a,a,sizeof(float)* (102));
	gettimeofday(&t2, 0);
